...
public class Problem860_lemonadeChange {
    ...
    public static boolean lemonadeChange(int[] bills) {
        ...
        for (int i = 0; i < bills.length; i++) {
            if (bills[i] == 5) {
                ...
                if (fiveDollarsNum < 1) {
                    return false;
                } else if (fiveDollarsNum < 3 && tenDollarsNum < 1) {
                    return false;
                } else if (tenDollarsNum >= 1 && fiveDollarsNum >= 1) {
                    tenDollarsNum--;
                    fiveDollarsNum--;
                } else {
                    fiveDollarsNum -= 3;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("true ?= " + lemonadeChange(new int[]{5, 5, 10}));
        System.out.println("false ?= " + lemonadeChange(new int[]{10, 10}));
        System.out.println("false ?= " + lemonadeChange(new int[]{5, 5, 10, 10, 20}));
        System.out.println("true ?= " + lemonadeChange(new int[]{5, 5, 5, 5, 10, 5, 10, 10, 10, 20}));
    }
}
