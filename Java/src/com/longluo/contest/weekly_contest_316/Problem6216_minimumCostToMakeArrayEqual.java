...
public class Problem6216_minimumCostToMakeArrayEqual {
    ...
    // TLE
    public static long minCost_opt(int[] nums, int[] cost) {
        int len = nums.length;
        ...
        for (int i = 0; i < len; i++) {
            ...
            for (int j = 0; j < len; j++) {
                ...
                sum += (long) Math.abs(nums[j] - nums[i]) * cost[j];
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("8 ?= " + minCost_opt(new int[]{1, 3, 5, 2}, new int[]{2, 3, 1, 14}));
    }
}
